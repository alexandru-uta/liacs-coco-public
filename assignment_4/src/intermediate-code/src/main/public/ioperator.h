...
namespace util
{ // Adding `to_string` implementation for IOperator, so we can use `util::to_string(ioperator)`.
     template <>
     std::string to_string<IOperator>(IOperator);
}

namespace std
{ // Adding hashfunction for IOperator to standard namespace, so we can use it in `unordered_map` etc.
     template <>
